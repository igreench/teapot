***

### Back
- [Простой чат на AioHTTP](https://maks.live/articles/python/prostoi-chat-na-aiohttp/)
- [Написание framework на asyncio, aiohttp и мысли про Python3 часть первая](https://habr.com/post/242541/)
- [Измерение производительности популярных python web-фреймворков](http://klen.github.io/python-web-benchmarks.html)
- [Как за 10 минут сделать клиент к HTTP API на Swagger](https://habr.com/company/Voximplant/blog/345148/)

***

### DB
- [Миллион строк в секунду из Postgres с помощью Python](https://habr.com/post/317394/)
- [Peewee – лёгкая, гибкая и очень быстрая ORM на Python](https://habr.com/post/207110/)
- [Peewee and Peewee-async: Why is async slower?](https://stackoverflow.com/questions/39803746/peewee-and-peewee-async-why-is-async-slower)

***

### Monitoring

- [Monitoring Your Synchronous Python Web Applications Using Prometheus](https://blog.codeship.com/monitoring-your-synchronous-python-web-applications-using-prometheus/)
- [Monitoring Your Asynchronous Python Web Applications Using Prometheus](https://blog.codeship.com/monitoring-your-asynchronous-python-web-applications-using-prometheus/)

***

### virtual env

- [virtual env cheet list](https://eax.me/python-virtualenv/)
- [What is the relationship between virtualenv and pyenv?](https://stackoverflow.com/a/29950604)

***

### snippets

- [Подборка @pythonetc, ноябрь 2018](https://habr.com/company/mailru/blog/432628/)

***

### language

- [Встроенные функции Python: какие нужно знать и на какие не стоит тратить время](https://tproger.ru/translations/python-built-ins-worth-learning/)

***
